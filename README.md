...
- **External Services**: Next, we check the timeline for calls made to outside services like a payment process. We look at how long these specific calls took to complete. If an outside service is taking too long to respond, we know the issue is external and the bottleneck is not in our own code.
